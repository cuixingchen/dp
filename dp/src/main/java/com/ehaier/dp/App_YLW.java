...
//			BigDecimal result = dealString(msg);
//			allResult = allResult.add(result);
		...
		// BigDecimal("100")).doubleValue());
		 ...
		 printExcle();
		System.out.println("完成allResult:" + allResult.doubleValue());
	 ...
	 * 打印
	 */
	private static void printExcle() {
		...
		titleList.add(new ExcelBean("正确的优惠", "yh"));
		titleList.add(new ExcelBean("当前优惠", "nyh"));
		titleList.add(new ExcelBean("正确付款金额", "fk"));
		titleList.add(new ExcelBean("当前付款金额", "nfk"));
		titleList.add(new ExcelBean("价格", "price"));
		titleList.add(new ExcelBean("数量", "num"));
		HSSFWorkbook file = PoiUtil.excelService("邮乐所有订单数据", titleList, contentList);
		try {
			FileOutputStream os = new FileOutputStream("d:\\邮乐所有完成订单数据汇总191单.xls");
			file.write(os);
			os.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static void dealJson(String msg) {
		...
		if ("youle".equals(key)) {
			...
			if(sourceOrderSn.equals("2015110516386705")){
				System.out.println("");
			}
		...
		if ("youle".equals(key)) {
			...
			for (int i = 0; i < count; i++) {
				...
				for (int m = 0; m < list.size(); m++) {
					OrderProducts orderpro = list.get(m);
					if (orderpro.getSku().equals(sku)) {
//						 result=result.add(orderpro.getCouponAmount());
//						 result=result.add(pCouponAmount);
						 result=result.add(productAmount);
//						 result=result.add(orderpro.getProductAmount());
					}
				}
			}
		}
